...
// TestWaitForSyncPublishesWhenHealthTransitionsWithNoFurtherEvent verifies
// the regression where acquisition health can become HEALTHY after all sync
// predicates are true even when no further Kubernetes object event occurs.
//
// The test intentionally avoids starting informer goroutines. It supplies a
// deterministic synced predicate and forces the last published snapshot to
// RESYNCING, then proves that a direct WaitForSync call republishes HEALTHY.
func TestWaitForSyncPublishesWhenHealthTransitionsWithNoFurtherEvent(t *testing.T) {
	...
	// Drain the forced publication so the next signal must come from
	// WaitForSync's HEALTHY transition.
	<-rt.state.Publications()
	...
	if latest.Acquisition() != clusterstate.AcquisitionHealthy || !latest.Trustworthy() {
		t.Fatalf(
			"Latest() acquisition = %s (trustworthy=%v) after WaitForSync, want HEALTHY/trustworthy",
			latest.Acquisition(),
			latest.Trustworthy(),
		)
	}
