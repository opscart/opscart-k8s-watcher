...
func azureRetailClient(t *testing.T, status int, body string, calls *int, expectedSKU, expectedRegion string) *http.Client {
	t.Helper()
	return &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		*calls++
		filter := r.URL.Query().Get("$filter")
		required := []string{"serviceName eq 'Virtual Machines'", "priceType eq 'Consumption'"}
		if expectedSKU != "" {
			required = append(required, "armSkuName eq '"+expectedSKU+"'")
		}
		if expectedRegion != "" {
			required = append(required, "armRegionName eq '"+expectedRegion+"'")
		}
...
func TestAzureRetailAPIIsCanonicalSourceForCatalogSKU(t *testing.T) {
	calls := 0
	body := `{"Items":[{"armSkuName":"Standard_D4s_v5","armRegionName":"centralus","serviceName":"Virtual Machines","productName":"Virtual Machines Dsv5 Series","skuName":"D4s v5","meterName":"D4s v5","type":"Consumption","unitOfMeasure":"1 Hour","currencyCode":"USD","retailPrice":0.42}]}`
	client := azureRetailClient(t, http.StatusOK, body, &calls, "Standard_D4s_v5", "centralus")
	...
	if err != nil || got.HourlyPrice != 0.42 {
		t.Fatalf("got %+v, err %v", got, err)
	}
	if got.Provenance != PriceProvenanceProviderAPIExact || got.Source != "Azure Retail Prices API" {
...
func TestAzureRetailPriceFilteringAndCache(t *testing.T) {
	calls := 0
	client := azureRetailClient(t, http.StatusOK, retailPriceItems, &calls, "Standard_E48as_v5", "centralus")
	...
	for i := 0; i < 2; i++ {
		...
		if err != nil || got.HourlyPrice != 2.75 || got.Currency != "USD" || got.Provenance != PriceProvenanceProviderAPIExact {
			t.Fatalf("got %+v, err %v", got, err)
		}
	}
	if calls != 1 {
...
func TestAzureRetailFailures(t *testing.T) {
	t.Run("server error", func(t *testing.T) {
		calls := 0
		client := azureRetailClient(t, http.StatusInternalServerError, `{}`, &calls, "Standard_E48as_v5", "centralus")
		provider := newAzurePricingProvider(client, azureRetailPricesURL, time.Hour)
	...
	t.Run("empty", func(t *testing.T) {
		calls := 0
		client := azureRetailClient(t, http.StatusOK, `{"Items":[]}`, &calls, "Standard_E48as_v5", "centralus")
		provider := newAzurePricingProvider(client, azureRetailPricesURL, time.Hour)
		_, err := provider.LookupOnDemandPrice(context.Background(), PriceRequest{InstanceType: "Standard_E48as_v5", Region: "centralus", OS: "linux"})
