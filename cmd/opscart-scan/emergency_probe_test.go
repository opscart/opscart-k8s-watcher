...
func TestHasProbeFailureSignature_MedispanDurEngineRealEvents(t *testing.T) {
	events := []string{
		"Startup probe failed: HTTP probe failed with statuscode: 500",
		"Container medispan-dur-engine failed startup probe, will be restarted",
	}
	if !hasProbeFailureSignature(events) {
		t.Errorf("expected the real medispan-dur-engine event text to match the probe-failure signature")
	}
}
